...
- If no GitHub remote: ask "Which GitHub repo should party.md manage? (e.g. `owner/repo`)" and set the remote.
...
> - [ ] **`/loop 5m /party`** — Run in your terminal. Claude Code CLI polls every 5 minutes. Free credit pool, easiest to start and stop. *(Recommended to start)*
...
> - [ ] **Branch + PR, you review** — Agent opens a PR on a `party/<issue-number>-<slug>` branch. You review and merge manually. *(Recommended)*
...
If a project named **"party.md"** already exists: use it. Record its `number` and `id` (node ID, `PVT_...`). Skip to 3c.
...
Write `.party.config.json` (substitute real values):
...
Or push any issue with the `party` label to trigger it.
```


## Phase 7: Tick Mode

*Runs on every `/party` call after setup. Also the handler for GitHub Actions triggers (`--issue <N>`) and `/party --status`.*
...
If missing: stop and tell the user to run `/party --setup`.

Extract: `owner`, `projectNumber`, `projectId`, `fields.status.*`, `settings.*`

### 7b. Sync untracked issues into the project

Fetch all open repo issues labeled `party` that are NOT yet in the project:

```bash
# All open issues with the party label
...
- Find the last `[party.md]` refinement comment (look for the marker string `**[party.md]**`)
...
>
> Do NOT close the issue — party handles that when the PR merges or direct commit is confirmed.
> Do NOT move the project status — party handles that in the next tick.
...
```bash
# Check for a completion comment from party
...
- **No PR, no `building` label, item last updated >60 minutes ago**: stale build detected. Post a comment and move back to Ready for retry:
  ```bash
  gh issue comment $NUM --body "**[party.md]** No build activity detected. Moving back to Ready for retry."
...
- **`CHANGES_REQUESTED`**: move back to In Progress with a note:
  ```bash
  gh issue comment $NUM --body "**[party.md]** PR has changes requested. Moving back to In Progress."
